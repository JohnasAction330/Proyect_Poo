...
export const getUserWithoutDashboard = async (req: Request, res: Response) => {
  const userId = req.params.id;
...
export const getUsersPerId = async (req: Request, res: Response) => {
...
export const updateProject = async (req: Request, res: Response) => {
  ...
  if (user) {
    const project = user.dashboard[0].projects.find((p) => p.id === projectId);
    if (project) {
      const { isFavorite, isShared, isDeleted, isRecent, isSave, files } =
        req.body;

      if (isFavorite != undefined) project.isFavorite = isFavorite;
      if (isShared != undefined) project.isShared = isShared;
      if (isDeleted != undefined) project.isDeleted = isDeleted;
      if (isRecent != undefined) project.isRecent = isRecent;
      if (isSave != undefined) project.isSave = isSave;
...
export const shareProjectWithUser = async (req: Request, res: Response) => {
  const userId = req.params.id;
  const projectId = req.params.projectId;
...
export const unshareProjectWithUser = async (req: Request, res: Response) => {
  const userId = req.params.id;
  const projectId = req.params.projectId; 
...
export const getDeletedProjects = async (req: Request, res: Response) => {
  const deletedProjects = await DeletedProject.find();
  res.json(deletedProjects);
};

export const deleteProjectFromUser = async (req: Request, res: Response) => {
  try {
    const userId = req.params.id;
    const projectId = req.params.projectId;
    const user = await User.findById(userId);
    ...
    const projectIndex = user.dashboard[0].projects.findIndex(
      (p) => p.id == projectId
    );
    if (projectIndex === -1) {
...
export const deleteProjectFromBin = async (req: Request, res: Response) => {
  const projectId = req.params.projectId;
